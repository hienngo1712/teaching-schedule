...
import { formatDate, formatDayOfWeek, calcAttendanceRate } from "@/lib/utils"
...
export function StudentScheduleView({
  ...
  const studentQuery = trpc.student.list.useQuery(
    { search: "" }, // We don't have a getById yet, but list is fine if we find it
  ...
  const summary = useMemo(() => {
    ...
    const rate = calcAttendanceRate(present, total - pending)
  ...
  return (
    <div ref={exportRef}>
      <Card className="mt-8 overflow-hidden border-slate-200 shadow-md">
